...
const emptyCompany: Omit<Company, "id" | "createdAt" | "updatedAt"> = { name: "", shortName: "", ownership: "央企", industry: "能源电力", website: "", recruitmentUrl: "", description: "", applicationLimit: "", deadline: "", notes: "" };
const emptyJob: Omit<Job, "id" | "companyId" | "createdAt" | "updatedAt"> = { title: "", batch: "", location: "", category: "", url: "", jd: "", status: "待投递", preferenceOrder: null, appliedAt: "", deadline: "", progress: "", resumeName: "", notes: "" };
type CompanyDraft = Omit<Company, "id" | "createdAt" | "updatedAt"> & Partial<Pick<Company, "id" | "createdAt" | "updatedAt">>;
type JobDraft = Omit<Job, "id" | "createdAt" | "updatedAt" | "preferenceOrder"> & { preferenceOrder: number | null | "" } & Partial<Pick<Job, "id" | "createdAt" | "updatedAt">>;
  ...
  return <main>
    ...
    <section className="workspace">
      ...
      <div className="content"><div className="toolbar"><div className="search">⌕<input aria-label="搜索企业" placeholder="搜索企业、简称或描述…" value={query} onChange={e => setQuery(e.target.value)}/></div><span>共 {shown.length} 家企业</span></div>
        <div className="company-list">{shown.map(company => { const ownJobs = (hasJobFilters ? filteredJobs : jobs).filter(j => j.companyId === company.id); const isOpen = expanded.includes(company.id); return <article key={company.id} className="company-card">
          <div className="company-row"><button className="chevron" aria-label={isOpen ? "收起岗位" : "展开岗位"} onClick={() => setExpanded(isOpen ? expanded.filter(x => x !== company.id) : [...expanded, company.id])}>{isOpen ? "⌄" : "›"}</button><div className="avatar">{company.shortName?.slice(0, 2) || company.name.slice(0, 2)}</div><div className="company-main"><div><h2>{company.name}</h2><span className={`badge ${company.ownership.includes("民营") ? "private" : "state"}`}>{company.ownership}</span></div><p>{company.description || "暂无企业描述"}</p></div><div className="meta"><span>{company.industry}</span><b>{`岗位限制：${company.applicationLimit.trim() || "未知"}`}</b></div><div className="row-actions"><a href={company.website.startsWith("http") ? company.website : `https://${company.website}`} target="_blank" rel="noreferrer">官网 ↗</a><button onClick={() => setCompanyModal(company)}>编辑</button><button className="danger" onClick={() => removeCompany(company)}>删除</button></div></div>
          {isOpen && <div className="jobs"><div className="jobs-head"><div><b>投递岗位</b><span>{ownJobs.length} 条记录</span></div><button onClick={() => setJobModal({ companyId: company.id })}>＋ 新增岗位</button></div>{ownJobs.length === 0 ? <div className="empty">还没有岗位记录，点击右上角新增第一条。</div> : <div className="job-table"><div className="job-tr job-th"><span>岗位 / 批次</span><span>地点</span><span>状态</span><span>志愿排序</span><span>截止日期</span><span>简历</span><span>操作</span></div>{ownJobs.map(job => <div className="job-tr" key={job.id}><span><b>{job.title}</b><small>{job.batch || "未填写批次"}</small></span><span>{job.location || "—"}</span><span><i className={`status s-${statuses.indexOf(job.status)}`}>{job.status}</i></span><span>{job.preferenceOrder == null ? "—" : `第 ${job.preferenceOrder} 志愿`}</span><span>{job.deadline || "—"}</span><span>{job.resumeName || "—"}</span><span className="job-actions">{job.url && <a target="_blank" rel="noreferrer" href={job.url}>JD ↗</a>}<button onClick={() => setJobModal({ companyId: company.id, job })}>编辑</button><button onClick={async () => { if (confirm("删除该岗位记录？")) { await deleteJob(job.id); refresh(); } }}>删除</button></span></div>)}</div>}</div>}
...
function CompanyDialog({ value, onClose, onSave }: { value?: Company; onClose: () => void; onSave: (v: Company) => void }) {
  const [form, setForm] = useState<CompanyDraft>(value || emptyCompany); const update = <K extends keyof CompanyDraft>(k: K, v: CompanyDraft[K]) => setForm({ ...form, [k]: v });
  function submit(e: FormEvent) { e.preventDefault(); onSave({ ...form, applicationLimit: String(form.applicationLimit ?? "").trim() }); }
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">企业父条目</p><h2>{value ? "编辑企业" : "新增企业"}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid"><Field label="企业全称 *"><input required value={form.name} onChange={e => update("name", e.target.value)}/></Field><Field label="企业简称"><input value={form.shortName} onChange={e => update("shortName", e.target.value)}/></Field><Field label="企业性质"><select value={form.ownership} onChange={e => update("ownership", e.target.value)}><option>央企</option><option>北京市属国企</option><option>其他国企</option><option>互联网民营企业</option><option>其他民营企业</option></select></Field><Field label="行业分类"><select value={form.industry} onChange={e => update("industry", e.target.value)}>{industries.slice(1).map(x => <option key={x}>{x}</option>)}</select></Field><Field label="集团官网 *"><input required placeholder="https://" value={form.website} onChange={e => update("website", e.target.value)}/></Field><Field label="招聘官网"><input placeholder="https://" value={form.recruitmentUrl} onChange={e => update("recruitmentUrl", e.target.value)}/></Field><Field label="志愿投递限制"><input type="text" placeholder="例如：限投 2 个岗位 / 不限" value={form.applicationLimit} onChange={e => update("applicationLimit", e.target.value)}/></Field><Field label="整体截止日期"><input type="date" value={form.deadline} onChange={e => update("deadline", e.target.value)}/></Field><Field label="企业描述" wide><textarea rows={3} value={form.description} onChange={e => update("description", e.target.value)}/></Field><Field label="其他备注" wide><textarea rows={2} value={form.notes} onChange={e => update("notes", e.target.value)}/></Field></div><div className="modal-foot"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary">保存企业</button></div></form></div>;
}

function JobDialog({ value, companyId, onClose, onSave }: { value?: Job; companyId: string; onClose: () => void; onSave: (v: Job) => void }) {
  const [form, setForm] = useState<JobDraft>(value || { ...emptyJob, companyId }); const update = <K extends keyof JobDraft>(k: K, v: JobDraft[K]) => setForm({ ...form, [k]: v });
  function submit(e: FormEvent) { e.preventDefault(); onSave({ ...form, preferenceOrder: form.preferenceOrder === "" ? null : Number(form.preferenceOrder), companyId }); }
  return <div className="modal-backdrop"><form className="modal wide-modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">岗位子条目</p><h2>{value ? "编辑投递岗位" : "新增投递岗位"}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid"><Field label="岗位名称 *"><input required value={form.title} onChange={e => update("title", e.target.value)}/></Field><Field label="招聘批次"><input placeholder="例如：2027 届秋招" value={form.batch} onChange={e => update("batch", e.target.value)}/></Field><Field label="工作地点"><input value={form.location} onChange={e => update("location", e.target.value)}/></Field><Field label="岗位类别"><input placeholder="研发 / 测试 / 产品" value={form.category} onChange={e => update("category", e.target.value)}/></Field><Field label="投递状态"><select value={form.status} onChange={e => update("status", e.target.value)}>{statuses.map(x => <option key={x}>{x}</option>)}</select></Field><Field label="志愿排序"><input type="number" min="1" step="1" placeholder="例如：1 表示第一志愿" value={form.preferenceOrder ?? ""} onChange={e => update("preferenceOrder", e.target.value)}/></Field><Field label="投递日期"><input type="date" value={form.appliedAt} onChange={e => update("appliedAt", e.target.value)}/></Field><Field label="岗位截止日期"><input type="date" value={form.deadline} onChange={e => update("deadline", e.target.value)}/></Field><Field label="简历附件/版本"><input placeholder="例如：后端开发-v3.pdf" value={form.resumeName} onChange={e => update("resumeName", e.target.value)}/></Field><Field label="岗位官网链接" wide><input type="url" placeholder="https://" value={form.url} onChange={e => update("url", e.target.value)}/></Field><Field label="当前面试进度" wide><input placeholder="例如：8 月 20 日完成一面，等待结果" value={form.progress} onChange={e => update("progress", e.target.value)}/></Field><Field label="JD" wide><textarea rows={7} value={form.jd} onChange={e => update("jd", e.target.value)}/></Field><Field label="其他备注" wide><textarea rows={3} value={form.notes} onChange={e => update("notes", e.target.value)}/></Field></div><div className="modal-foot"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary">保存岗位</button></div></form></div>;
}
