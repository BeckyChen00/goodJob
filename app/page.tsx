...
type JobDraft = Omit<Job, "id" | "createdAt" | "updatedAt"> & Partial<Pick<Job, "id" | "createdAt" | "updatedAt">>;
...
function CompanyDialog({ value, onClose, onSave }: { value?: Company; onClose: () => void; onSave: (v: Company) => void }) {
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><div className="modal-head"><div><p className="eyebrow">企业父条目</p><h2>{value ? "编辑企业" : "新增企业"}</h2></div><button type="button" onClick={onClose}>×</button></div><div className="form-grid"><Field label="企业全称 *"><input required value={form.name} onChange={e => update("name", e.target.value)}/></Field><Field label="企业简称"><input value={form.shortName} onChange={e => update("shortName", e.target.value)}/></Field><Field label="企业性质"><select value={form.ownership} onChange={e => update("ownership", e.target.value)}><option>央企</option><option>北京市属国企</option><option>其他国企</option><option>互联网民营企业</option><option>其他民营企业</option></select></Field><Field label="行业分类"><select value={form.industry} onChange={e => update("industry", e.target.value)}>{industries.slice(1).map(x => <option key={x}>{x}</option>)}</select></Field><Field label="集团官网 *"><input required placeholder="https://" value={form.website} onChange={e => update("website", e.target.value)}/></Field><Field label="招聘官网"><input placeholder="https://" value={form.recruitmentUrl} onChange={e => update("recruitmentUrl", e.target.value)}/></Field><Field label="志愿投递限制"><input type="number" min="0" placeholder="未知时留空" value={form.applicationLimit ?? ""} onChange={e => update("applicationLimit", e.target.value)}/></Field><Field label="整体截止日期"><input type="date" value={form.deadline} onChange={e => update("deadline", e.target.value)}/></Field><Field label="企业描述" wide><textarea rows={3} value={form.description} onChange={e => update("description", e.target.value)}/></Field><Field label="其他备注" wide><textarea rows={2} value={form.notes} onChange={e => update("notes", e.target.value)}/></Field></div><div className="modal-foot"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary">保存企业</button></div></form></div>;
}
