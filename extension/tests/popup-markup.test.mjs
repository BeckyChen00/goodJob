...
const damagedPatterns = [/\?input\b/i, /\?textarea\b/i, /\?\/title/i, /\?\/h1/i, /\ufffd/, /娴嬭瘯|鐮斿彂|浜у搧|鎶曢€掕崏绋?/];
...
async function assertAssets(root, label) {
  ...
  assert.ok(!damagedPatterns.some((pattern) => pattern.test(manifestText)), `${label} manifest has no damaged text`);
  ...
  assert.equal((html.match(/<textarea\b/gi) ?? []).length, 2, `${label} has 2 textareas`);
  ...
  assert.match(html, /岗位匹配/, `${label} exposes job fit action`);
  assert.match(html, /流式调用 LLM/, `${label} explains browser-side streaming`);
